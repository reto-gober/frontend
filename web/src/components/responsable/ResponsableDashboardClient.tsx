...
import { reportesService, flujoReportesService, type ReporteResponse } from '../../lib/services';
  ...
  const cargarDatos = async () => {
    try {
      ...
      const enProceso = reportes.filter(r => 
        r.estado === 'EN_ELABORACION' || r.estado === 'EN_REVISION'
      ).length;
      ...
      const reportesConVencimiento = reportes
        .filter(r => r.fechaVencimiento && r.estado !== 'COMPLETADO' && r.estado !== 'ENVIADO')
        .map(r => {
          const fechaVenc = new Date(r.fechaVencimiento!);
      ...
      <div className="dashboard-grid">
        ...
        <div className="dashboard-card wide">
          <div className="card-header">
            ...
            <a href="/roles/responsable/mis-reportes" className="card-link">Ver todos →</a>
          </div>
          <div className="card-body">
                  ...
                  <div key={idx} className="vencimiento-item">
                    <div className="vencimiento-info">
                      <div className="vencimiento-header">
                        <span className="vencimiento-codigo">{reporte.codigo}</span>
                        <span className={`dias-badge ${reporte.diasRestantes <= 3 ? 'urgent' : 'normal'}`}>
                      ...
                      <p className="vencimiento-meta">
                        ...
                        <span>Vence: {formatFecha(reporte.fechaVencimiento)}</span>
                      </p>
                    </div>
                    ...
                    <span className="progress-label">{reporte.progreso}% completado</span>
                  </div>
                ))}
              </div>
            )}
