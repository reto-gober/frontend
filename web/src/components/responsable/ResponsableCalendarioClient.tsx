...
export default function ResponsableCalendarioClient() {
  ...
  const cargarCalendario = async () => {
    try {
      ...
      console.log("🔄 [Calendario] Cargando calendario del responsable...");
      ...
      console.log("📅 [Calendario] Rango de fechas:", {
        fechaInicio,
        fechaFin,
      });
      ...
      console.log(
        "📊 [Calendario] Total de eventos:",
        response?.eventos?.length || 0
      );
      ...
      console.log("✅ [Calendario] Calendario cargado exitosamente");
    } catch (err: any) {
      console.error("❌ [Calendario] Error al cargar calendario:", err);
      console.error("❌ [Calendario] Respuesta del error:", err.response?.data);
      console.error("❌ [Calendario] Status del error:", err.response?.status);

      const mensajeError =
        err.response?.data?.message ||
        err.message ||
        "Error al cargar tu calendario. Verifica la consola.";
  ...
  const monthName = mesActual.toLocaleDateString("es-CO", {
    month: "long",
    year: "numeric",
  });
        ...
        <div className="header-info">
          ...
          {calendario && (
            ...
            >
              <span>
                Mis Pendientes:{" "}
                <strong>{calendario.misReportesPendientes || 0}</strong>
              </span>
              <span>
                Enviados: <strong>{calendario.misReportesEnviados || 0}</strong>
              </span>
            </div>
          )}
        </div>
        <div className="header-actions">
          <button className="btn-secondary" onClick={irAHoy}>
            Hoy
          </button>
        </div>
      </div>
            ...
            <div className="eventos-list">
              {proximosEventos.length === 0 ? (
                ...
                proximosEventos.map((evento, idx) => {
                  ...
                  const fechaRef =
                    evento.date || evento.fechaVencimiento || evento.endDate;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="evento-info">
                        ...
                        <span
                          className={`evento-badge ${urgente ? "urgent" : warning ? "warning" : ""}`}
                        >
                          {dias === 0
                            ? "Hoy"
                            : dias === 1
                        ...
                        {eventosDelDia.slice(0, 2).map((evento, idx) => {
                          ...
                          const esVencimiento =
                            ...
                            (evento.fechaVencimiento &&
                              evento.fechaVencimiento.split("T")[0] ===
                                fechaDia) ||
                            (evento.date &&
                              evento.date.split("T")[0] === fechaDia);
                          const esPeriodo = evento.tipo === "periodo";
                    ...
                    {getEventosDelDia(diaSeleccionado).map((evento, idx) => (
                      ...
                      >
                        ...
                        >
                          {evento.tipo} -{" "}
                          <span
                            style={{ color: evento.color, fontWeight: 500 }}
                          >
                            {evento.estado}
                          </span>
                        </div>
