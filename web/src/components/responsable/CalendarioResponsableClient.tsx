...
export default function CalendarioResponsableClient() {
  ...
  const [filtrosEntidades, setFiltrosEntidades] = useState<Set<string>>(new Set());
  ...
  const loadEventos = async () => {
    try {
      ...
      periodos.forEach(periodo => {
        ...
        let tipo: 'pendiente' | 'enviado' | 'vencido' | 'porVencer' = 'pendiente';
  ...
  const proximosVencimientos = eventos
    .filter(e => e.tipo === 'vencido' || e.tipo === 'porVencer' || e.tipo === 'pendiente')
  ...
  const eventosDelDia = diaSeleccionado
    ? eventos.filter(e => {
        ...
        return eventoFecha.getDate() === diaSeleccionado.getDate() &&
  ...
  const generarDiasDelMes = () => {
    ...
    const dias: Array<{fecha: Date, esDelMes: boolean, eventos: EventoCalendario[]}> = [];
  ...
  const getColorEvento = (tipo: string) => {
    switch(tipo) {
      case 'vencido': return 'var(--error-red-500)';
      case 'porVencer': return 'var(--warning-yellow-500)';
      case 'enviado': return 'var(--success-green-500)';
      case 'pendiente': return 'var(--role-accent)';
            ...
            <div className="eventos-list">
              {proximosVencimientos.length === 0 ? (
                ...
                proximosVencimientos.map(evento => {
                  ...
                  const esUrgente = evento.tipo === 'vencido' || evento.tipo === 'porVencer';
                  ...
                  return (
                    ...
                    >
                      <div className="evento-date">
                        <span className="date-day">{evento.fecha.getDate()}</span>
                        <span className="date-month">
                          {evento.fecha.toLocaleDateString('es-CO', { month: 'short' })}
                        </span>
                      </div>
                      <div className="evento-info">
                        <h4 className="evento-title">{evento.titulo}</h4>
                        <p className="evento-description">{evento.entidad}</p>
                        <span className={`evento-badge ${evento.tipo === 'vencido' ? 'urgent' : evento.tipo === 'porVencer' ? 'warning' : ''}`}>
                          {evento.tipo === 'vencido' 
        ...
        <div className="calendario-main">
          <div className="calendario-card">
            ...
            <div className="calendar-nav">
              <button className="nav-btn" onClick={() => vista === 'semana' ? cambiarSemana('prev') : cambiarMes('prev')}>
              ...
              {vista !== 'lista' && (
                <button className="nav-btn" onClick={() => vista === 'semana' ? cambiarSemana('next') : cambiarMes('next')}>
                  <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2">
                    <polyline points="9 18 15 12 9 6"/>
                  </svg>
                </button>
              )}
              <button className="btn-today" onClick={irAHoy}>Hoy</button>
            </div>
              ...
              <>
                ...
                <div className="calendar-grid">
                  {diasDelMes.map((dia, index) => {
                    ...
                    const tieneVencido = dia.eventos.some(e => e.tipo === 'vencido');
                    const tienePorVencer = dia.eventos.some(e => e.tipo === 'porVencer');
                  ...
                  return (
                    ...
                    >
                      <div className="week-day-header">
                        ...
                        <span className={`week-day-number ${hoy ? 'today' : ''}`}>
                          {dia.fecha.getDate()}
                        </span>
                      </div>
                      <div className="week-day-events">
                        {dia.eventos.length === 0 ? (
                          ...
                          dia.eventos.map(evento => (
                            <div key={evento.periodoId} className={`week-event ${evento.tipo}`}>
                              ...
                              <div className="week-event-title">{evento.titulo}</div>
                              <div className="week-event-entity">{evento.entidad}</div>
                            </div>
                          ))
                        )}
                      </div>
              ...
              <div className="list-view">
                {eventos.length === 0 ? (
                  ...
                  [...eventos]
                    .sort((a, b) => a.fecha.getTime() - b.fecha.getTime())
                    .map(evento => (
                      <div key={evento.periodoId} className={`list-event ${evento.tipo}`}>
                        <div className="list-event-date">
                          <span className="list-date-day">{evento.fecha.getDate()}</span>
          ...
          {vista !== 'lista' && (
            ...
            <div className="detail-header">
              <h3>{diaSeleccionado ? formatearFecha(diaSeleccionado) : 'Selecciona un día'}</h3>
            ...
            <div className="detail-events">
              {eventosDelDia.length === 0 ? (
                ...
                eventosDelDia.map(evento => (
                  <div key={evento.periodoId} className={`detail-event ${evento.tipo}`}>
