import { useState, useEffect } from 'react';
import { ProtectedRoute } from '../common/ProtectedRoute';
import { TarjetaPeriodo } from '../flujo/TarjetaPeriodo';
import { ModalValidarReporte } from '../modales/ModalValidarReporte';
import { flujoReportesService, type ReportePeriodo } from '../../lib/services';
import { useToast, ToastContainer } from '../Toast';

type TabType = 'pendientes' | 'aprobados' | 'rechazados';

export default function PendientesValidacionPage() {
  const [activeTab, setActiveTab] = useState<TabType>('pendientes');
  const [periodos, setPeriodos] = useState<ReportePeriodo[]>([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(0);
  const [totalPages, setTotalPages] = useState(0);
  const [modalValidar, setModalValidar] = useState<{
    isOpen: boolean;
    periodoId: string;
    reporteNombre: string;
    responsable: string;
  }>({ isOpen: false, periodoId: '', reporteNombre: '', responsable: '' });
  const { toasts, removeToast, success, error } = useToast();

  const loadPeriodos = async () => {
    try {
      setLoading(true);
      let response;

      switch (activeTab) {
        case 'pendientes':
          response = await flujoReportesService.pendientesValidacion(page, 10);
          break;
        case 'aprobados':
          response = await flujoReportesService.porEstado('aprobado', page, 10);
          break;
        case 'rechazados':
          response = await flujoReportesService.porEstado('rechazado', page, 10);
          break;
      }

      setPeriodos(response.content);
      setTotalPages(response.totalPages);
    } catch (err) {
      error('Error al cargar los reportes');
      console.error(err);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadPeriodos();
  }, [activeTab, page]);

  const handleAccion = async (accion: string, periodoId: string) => {
    const periodo = periodos.find(p => p.periodoId === periodoId);
    if (!periodo) return;

    if (accion === 'ver') {
      window.location.href = `/validacion/${periodoId}`;
      return;
    }

    if (accion === 'validar') {
      setModalValidar({
        isOpen: true,
        periodoId,
        reporteNombre: periodo.reporteNombre,
        responsable: periodo.responsableElaboracion.nombreCompleto
      });
      return;
    }
  };

  const handleValidacionExitosa = () => {
    success('Validación realizada exitosamente');
    loadPeriodos();
  };

  const tabs = [
    { id: 'pendientes' as TabType, label: 'Pendientes', count: 0 },
    { id: 'aprobados' as TabType, label: 'Aprobados', count: 0 },
    { id: 'rechazados' as TabType, label: 'Rechazados', count: 0 },
  ];

  return (
    <ProtectedRoute allowedRoles={['supervisor', 'admin']}>
      <ToastContainer toasts={toasts} onClose={removeToast} />
      
      <ModalValidarReporte
        periodoId={modalValidar.periodoId}
        reporteNombre={modalValidar.reporteNombre}
        responsable={modalValidar.responsable}
        isOpen={modalValidar.isOpen}
        onClose={() => setModalValidar({ ...modalValidar, isOpen: false })}
        onSuccess={handleValidacionExitosa}
        onError={error}
      />

      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        {/* Header */}
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '1.875rem', fontWeight: '700', color: 'var(--neutral-900)', margin: '0' }}>
            Validación de Reportes
          </h1>
          <p style={{ fontSize: '0.875rem', color: 'var(--neutral-500)', marginTop: '0.5rem' }}>
            Revisa y valida los reportes enviados por los responsables
          </p>
        </div>

        {/* Tabs */}
        <div style={{
          display: 'flex',
          gap: '1rem',
          borderBottom: '2px solid var(--neutral-200)',
          marginBottom: '2rem'
        }}>
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
                setActiveTab(tab.id);
                setPage(0);
              }}
              style={{
                padding: '0.75rem 1.5rem',
                background: 'none',
                border: 'none',
                borderBottom: activeTab === tab.id ? '2px solid var(--color-primary-600)' : '2px solid transparent',
                color: activeTab === tab.id ? 'var(--color-primary-600)' : 'var(--neutral-600)',
                fontWeight: activeTab === tab.id ? '600' : '500',
                fontSize: '0.875rem',
                cursor: 'pointer',
                marginBottom: '-2px',
                transition: 'all 0.15s ease',
                display: 'flex',
                alignItems: 'center',
                gap: '0.5rem'
              }}
            >
              {tab.label}
              {tab.count > 0 && (
                <span style={{
                  background: activeTab === tab.id ? 'var(--color-primary-600)' : 'var(--neutral-400)',
                  color: 'white',
                  padding: '0.125rem 0.5rem',
                  borderRadius: '9999px',
                  fontSize: '0.75rem',
                  fontWeight: '600'
                }}>
                  {tab.count}
                </span>
              )}
            </button>
          ))}
        </div>

        {/* Content */}
        {loading ? (
          <div style={{ 
            display: 'flex', 
            justifyContent: 'center', 
            alignItems: 'center', 
            minHeight: '300px' 
          }}>
            <div style={{
              width: '40px',
              height: '40px',
              border: '4px solid var(--neutral-200)',
              borderTop: '4px solid var(--color-primary-600)',
              borderRadius: '50%',
              animation: 'spin 1s linear infinite'
            }} />
          </div>
        ) : periodos.length === 0 ? (
          <div style={{
            textAlign: 'center',
            padding: '3rem 1rem',
            backgroundColor: 'var(--neutral-50)',
            borderRadius: '0.75rem',
            border: '2px dashed var(--neutral-300)'
          }}>
            <svg 
              style={{ margin: '0 auto 1rem', color: 'var(--neutral-400)' }}
              width="64" 
              height="64" 
              viewBox="0 0 24 24" 
              fill="none" 
              stroke="currentColor" 
              strokeWidth="2"
            >
              <path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <h3 style={{ fontSize: '1.125rem', fontWeight: '600', color: 'var(--neutral-900)', marginBottom: '0.5rem' }}>
              {activeTab === 'pendientes' && 'No hay reportes pendientes'}
              {activeTab === 'aprobados' && 'No hay reportes aprobados'}
              {activeTab === 'rechazados' && 'No hay reportes rechazados'}
            </h3>
            <p style={{ fontSize: '0.875rem', color: 'var(--neutral-500)' }}>
              {activeTab === 'pendientes' && 'Todos los reportes han sido validados'}
              {activeTab === 'aprobados' && 'Aún no has aprobado ningún reporte'}
              {activeTab === 'rechazados' && 'Aún no has rechazado ningún reporte'}
            </p>
          </div>
        ) : (
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {periodos.map(periodo => (
                <TarjetaPeriodo
                  key={periodo.periodoId}
                  periodo={periodo}
                  onAccion={handleAccion}
                  mostrarResponsables={true}
                />
              ))}
            </div>

            {/* Pagination */}
            {totalPages > 1 && (
              <div style={{
                display: 'flex',
                justifyContent: 'center',
                gap: '0.5rem',
                marginTop: '2rem'
              }}>
                <button
                  onClick={() => setPage(p => Math.max(0, p - 1))}
                  disabled={page === 0}
                  style={{
                    padding: '0.5rem 1rem',
                    backgroundColor: page === 0 ? 'var(--neutral-100)' : 'white',
                    border: '1px solid var(--neutral-300)',
                    borderRadius: '0.5rem',
                    cursor: page === 0 ? 'not-allowed' : 'pointer',
                    fontSize: '0.875rem',
                    fontWeight: '500',
                    color: page === 0 ? 'var(--neutral-400)' : 'var(--neutral-700)'
                  }}
                >
                  Anterior
                </button>
                <span style={{
                  display: 'flex',
                  alignItems: 'center',
                  padding: '0 1rem',
                  fontSize: '0.875rem',
                  color: 'var(--neutral-600)'
                }}>
                  Página {page + 1} de {totalPages}
                </span>
                <button
                  onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))}
                  disabled={page >= totalPages - 1}
                  style={{
                    padding: '0.5rem 1rem',
                    backgroundColor: page >= totalPages - 1 ? 'var(--neutral-100)' : 'white',
                    border: '1px solid var(--neutral-300)',
                    borderRadius: '0.5rem',
                    cursor: page >= totalPages - 1 ? 'not-allowed' : 'pointer',
                    fontSize: '0.875rem',
                    fontWeight: '500',
                    color: page >= totalPages - 1 ? 'var(--neutral-400)' : 'var(--neutral-700)'
                  }}
                >
                  Siguiente
                </button>
              </div>
            )}
          </>
        )}
      </div>
    </ProtectedRoute>
  );
}
