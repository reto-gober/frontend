...
import { flujoReportesService, evidenciasService, type EnviarReporteRequest } from '../../lib/services';
    ...
    try {
      ...
      if (archivos.length > 0) {
        ...
        const uploadPromises = archivos.map(async (archivo) => {
          const response = await evidenciasService.subir(periodoId, archivo);
          return response.id;
        });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label htmlFor="comentarios" style={{
              ...
              Comentarios {esCorreccion && <span style={{ color: 'var(--color-danger)' }}>*</span>}
            </label>
            <textarea
              id="comentarios"
          ...
          <div style={{
            ...
            <button
              ...
              disabled={enviando || uploading || (esCorreccion && !comentarios.trim())}
            >
              {uploading ? (
                <>
