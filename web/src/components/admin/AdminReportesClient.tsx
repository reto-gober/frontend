...
export default function AdminReportesClient() {
  ...
  const [editingReporteId, setEditingReporteId] = useState<string | undefined>(
    undefined
  );
  ...
  const frecuenciaLabel = (frecuencia?: string) => {
    ...
    return (
      map[key] ||
      frecuencia.charAt(0).toUpperCase() + frecuencia.slice(1).toLowerCase()
    );
  };
  ...
  const obtenerEstadoTemporal = (
    periodo: ReportePeriodo
  ): PeriodoDecorado["estadoTemporal"] => {
    const estadoLower = (periodo.estado || "").toLowerCase();
    if (periodo.fechaEnvioReal || estadoLower.includes("enviado"))
      return "enviado";
  ...
  const elegirPeriodoPreferido = (periodos: PeriodoDecorado[]) => {
    ...
    const calcularDistancia = (p: PeriodoDecorado) => {
      ...
      if (p.estadoTemporal === "futuro")
        return Math.abs(inicio - ahora.getTime());
      if (p.estadoTemporal === "vencido")
        return Math.abs(ahora.getTime() - fin);
      return 0;
    };
  ...
  const calcularEstadisticas = () => {
    ...
    setEstadisticas({
      ...
      enProgreso: reportes.filter((r) => r.estado?.toLowerCase() === "activo")
        .length,
      completados: 0, // No aplica a nivel de reporte
      vencidos: reportes.filter(
        (r) =>
          r.estado?.toLowerCase() === "cancelado" ||
          r.estado?.toLowerCase() === "suspendido"
      ).length,
    });
  };
  ...
  const abrirDetalleReporte = async (reporte: ReporteResponse) => {
    try {
      ...
      if (periodosDecorados.length === 0) {
        notifications.info(
          "Este reporte no tiene períodos disponibles para mostrar."
        );
        setModalSeleccionPeriodo({
          open: false,
          loading: false,
          reporteNombre: "",
          periodos: [],
  ...
  const abrirPanelCarga = async (
    ...
    try {
      ...
      const seleccionado =
        preseleccion || elegirPeriodoPreferido(periodosDecorados);

      if (!seleccionado && periodosDecorados.length > 0) {
        notifications.info(
          "Todos los periodos de este reporte ya fueron enviados."
        );
      }
  ...
  const lanzarEnvio = () => {
    const periodoSeleccionado = panelCarga.periodos.find(
      (p) => p.periodoId === panelCarga.seleccionado
    );
    if (
      !periodoSeleccionado ||
      periodoSeleccionado.estadoTemporal === "enviado"
    ) {
    ...
    setModalEnviar({
      ...
      frecuencia:
        periodoSeleccionado.frecuencia || periodoSeleccionado.periodoTipo || "",
    });
  };
    ...
    <div className="reportes-page">
      ...
      <div className="page-header">
        <div className="header-info">
          ...
          <p className="page-description">
            Vista global de todos los reportes del sistema
          </p>
        </div>
            ...
            {filteredReportes.length === 0 ? (
              ...
              filteredReportes.map((reporte) => {
                const isHighlighted =
                  ...
                  highlightReporteId === (reporte as any).periodoId;

                return (
                  <tr
                    key={reporte.reporteId}
                    id={`reporte-${reporte.reporteId}`}
                    style={
                      isHighlighted
                        ? {
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`status-badge ${getEstadoBadgeClass(reporte.estado)}`}
                      >
                        {reporte.estado}
                      </span>
                    </td>
                    <td>
                      <div className="action-buttons">
                        ...
                        <button
                          ...
                          title="Eliminar"
                          onClick={() => handleDeleteReporte(reporte.reporteId)}
                        >
                          <svg
                            viewBox="0 0 24 24"
                            width="18"
                        ...
                        <button
                          className="btn-icon"
                          title="Intervenir / Cargar"
                          onClick={() =>
                            abrirPanelCarga(reporte.reporteId, reporte.nombre)
                          }
                        >
                          <svg
                            viewBox="0 0 24 24"
              ...
              <div>
                ...
                {panelCarga.seleccionado && (
                  <div className="side-panel-meta">
                    <span className="chip-estado info">
                      {panelCarga.periodos.find(
                        (p) => p.periodoId === panelCarga.seleccionado
                      )?.frecuencia ||
                        panelCarga.periodos.find(
                          (p) => p.periodoId === panelCarga.seleccionado
            ...
            {panelCarga.loading ? (
              ...
              <div className="side-panel-body">
                <label className="side-panel-label">
                  Selecciona el periodo a intervenir
                </label>
                <select
                ...
                >
                  <span
                    style={{ fontSize: "0.75rem", color: "var(--neutral-600)" }}
                  >
                    Activos se seleccionan automáticamente. Enviados se
                    deshabilitan.
                  </span>
                </div>
      ...
      {modalSeleccionPeriodo.open && (
        <div
          ...
          onClick={cerrarModalSeleccionPeriodo}
        >
          <div
            className="modal-selector-periodo"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header del Modal */}
            <div className="modal-selector-header">
              <div className="modal-selector-header-content">
                ...
                <p className="modal-selector-subtitle">
                  {modalSeleccionPeriodo.reporteNombre}
                </p>
                <span className="modal-selector-count">
                  {modalSeleccionPeriodo.periodos.length} período
                  {modalSeleccionPeriodo.periodos.length !== 1 ? "s" : ""}{" "}
                  disponible
                  {modalSeleccionPeriodo.periodos.length !== 1 ? "s" : ""}
                </span>
              </div>
              <button
                className="modal-selector-close"
                onClick={cerrarModalSeleccionPeriodo}
                aria-label="Cerrar"
              >
                <svg
                ...
                <div className="modal-selector-body">
                  <div className="periodos-lista-scroll">
                    {modalSeleccionPeriodo.periodos.map((p) => {
                      const isSelected =
                        modalSeleccionPeriodo.seleccionado === p.periodoId;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="periodo-radio">
                            <div
                              className={`radio-button ${isSelected ? "radio-checked" : ""}`}
                            >
                          ...
                          <div className="periodo-content">
                            ...
                            <div className="periodo-header-line">
                              <span className="periodo-tipo">
                                {frecuenciaLabel(p.frecuencia || p.periodoTipo)}
                              </span>
                              <span
                                className={`periodo-estado-chip ${estadoClass}`}
                              >
                                {estadoLabel}
                              </span>
                            </div>
                            ...
                            <div className="periodo-fechas-line">
                              ...
                              <span>
                                {formatFechaCorta(p.periodoInicio)} →{" "}
                                {formatFechaCorta(p.periodoFin)}
                              </span>
                            </div>
